...
class SweetAlert: UIViewController {
    ...
    let kBakcgroundTansperancy: CGFloat = 0.8
    ...
    let kButtonHeight: CGFloat = 15.0
    ...
    func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            _ = subTitleTextView.text! as NSString
            ...
            textViewHeight = ceil(kMaxHeight)
        ...
        if buttons.count == 2 {
            ...
            totalWidth = buttonRect[0].size.width + 10.0
        }
        y += kHeightMargin
